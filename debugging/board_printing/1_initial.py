	...
	if player == 1:
		for m in reversed(range(0,6)):
			if  (connect4_board[m][column] == " "):
				...
				connect4_board[m][column] = 1
				return True

			elif (connect4_board[m][column] == 1) or (connect4_board[m][column] == 2):
				return False
	
	if player == 2:
		for n in reversed(range(0,6)):
			if  (connect4_board[n][column] == 0):
				connect4_board[n][column] = 2
				return True

			elif  (connect4_board[n][column] == 1) or (connect4_board[n][column] == 2):
				return False
def execute_player_turn(player, board): # Task 5
	# Implement your solution below
	if player == 1:
		prompt = "Player 1, please enter the column you would like to drop your piece into: "

	if player == 2:
		prompt = "Player 2, please enter the column you would like to drop your piece into: "
...
def end_of_game(board): # Question 6
	...
	for i in reversed(range(0,columns-3)): 
		...
		for j in reversed(range(0,rows)):
			...
			if board[j][i] == board[j][i+1] == board[j][i+2] == board[j][i+3] == 1:

				return 1
			#If the player is player 2:

			elif board[j][i] == board[j][i+1] == board[j][i+2] == board[j][i+3] == 2:
	...
	for i in range(0, columns - 3): 
		for j in range(3, rows): 
			...
			if board[j][i] == board[j-1][i+1] == board[j-2][i+2] == board[j-3][i+3] == 1:
				return 1

			elif board[j][i] == board[j-1][i+1] == board[j-2][i+2] == board[j-3][i+3] == 2:
				return 2
...
def local_2_player_game():
	...
	while True:
		...
		if ((dropped == True) and (player == 1)):
			
			player = 2

		elif ((dropped == True) and (player == 2)):
			player = 1
